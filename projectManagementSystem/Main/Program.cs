...
namespace projectManagementSystem.Main
{
    public class Program
    {
        private static async Task Main(string[] args)
        {
            ...
            await UtilsPasswordMigrationHelper.HashAndUpdatePasswordForUser(userRepo, "admin1", "qwe", cancellationToken); //использовал для обхода проблемы с паролем самого первого пользователя
            while (true)
            {
